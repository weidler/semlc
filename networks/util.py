...
def prepare_lc_builder(setting: str, ricker_width: float, ricker_damp: float) -> Callable:
...
def build_network(network: str, input_shape: Tuple[int, int, int], n_classes: int = None, lc: Callable = None,
    ...
    if network in ["simple", Simple.__name__.lower()]:
        model = Simple(input_shape=input_shape, n_classes=n_classes, lateral_layer=lc,
                       **(dict(conv_one_init_std=init_std) if init_std is not None else dict()))
    elif network in ["shallow", Shallow.__name__.lower()]:
        model = Shallow(input_shape=input_shape, n_classes=n_classes, lateral_layer=lc)
    elif network in ["alexnet", AlexNet.__name__.lower()]:
        model = AlexNet(input_shape=input_shape, n_classes=n_classes, lateral_layer=lc)
    elif network in ["capsnet", CapsNet.__name__.lower()]:
        model = CapsNet(input_shape=input_shape, n_classes=n_classes, lateral_layer=lc)
    else:
...
if __name__ == '__main__':
    ...
    ssb_net = Shallow(input_shape=(3, 32, 32), n_classes=10)
