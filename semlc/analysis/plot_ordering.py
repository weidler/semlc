...
from utilities.filter_ordering import two_opt, mse, mae
...
def plot_ordering(net, plot_sequence=False, num_layer=0, save=True, point_size=4):
    ...
    if save:
        fig.savefig('./_ordering.pdf', format="pdf", bbox_inches='tight')
    plt.show()
...
def mse_difference(filters, scaler=None):
    """
    calculates the mse mse_differences between filters_per_group

    :param filters:         a tensor of filters_per_group (C X H X W) where C is the number of filters_per_group and H and W are spatial dimensions.
    :param scaler:          an optional sklearn scaler_mse to transform the mse_differences

    :return:                the mse difference between filters_per_group
    """
    differences = []
...
def mae_difference(filters, scaler=None):
    """
    calculates the mse mse_differences between filters_per_group
    ...
    differences = []
    for i in range(-1, len(filters) - 1):
        diff = mae(filters[i + 1], filters[i])
        differences.append(diff)
    if scaler is not None:
        differences = scaler.transform(np.array(differences).reshape(-1, 1))[:, 0].tolist()
    return sum(differences) / len(differences)
