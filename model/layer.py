...
class RecurrentInhibition(nn.Module):
    ...
    def _plot_inhibition_convergence(self, signal_in: torch.Tensor, signal_inhib: torch.Tensor, signal_rec: torch.Tensor, step_differences: List[float]):
        x = list(range(self.scope))

        self.axs_convergence[0].cla()
        self.axs_convergence[1].cla()

        self.axs_convergence[1].set_xlim([0, self.max_steps])
        self.axs_convergence[1].set_ylim([0, 0.03])
        ...
        self.axs_convergence[1].plot([self.convergence_threshold for _ in list(range(self.max_steps))], label="Convergence Threshold", color="red")

        self.axs_convergence[0].legend()
        self.axs_convergence[1].legend()
